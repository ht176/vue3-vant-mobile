...
export class TmplQuestionnaireItemServiceProxy extends ServiceProxyBase {
     ...
     * @return Success
     */
    tmplQuestionnaireItemPOST3(tmplQuestionnaireItemId: string , cancelToken?: CancelToken | undefined): Promise<Result> {
        let url_ = this.baseUrl + "/api/TmplQuestionnaireItem/{tmplQuestionnaireItemId}";
        ...
        return this.transformOptions(options_).then(transformedOptions_ => {
            ...
            return this.transformResult(url_, _response, (_response: AxiosResponse) => this.processTmplQuestionnaireItemPOST3(_response));
        });
    }

    protected processTmplQuestionnaireItemPOST3(response: AxiosResponse): Promise<Result> {
        const status = response.status;
        let _headers: any = {};
        if (response.headers && typeof response.headers === "object") {
            for (const k in response.headers) {

...
import type { PatientCardsView } from '@/services/PatientServiceProxies'
import { PatientCardsServiceProxy, PatientView } from '@/services/PatientServiceProxies'

const { patient } = defineProps({
  patient: PatientView,
})
const tableData = ref<PatientCardsView[]>([])
const ruleFormRef = ref<FormInstance>()
const loading = ref(false)
const rowRecord = ref({})

const ruleCardNumber = reactive<FormRules<PatientCardsView>>({
  cardNumber: [
    { required: true, message: '请输入透析卡号', trigger: 'change' },
...
const patientCardsServiceProxy = new PatientCardsServiceProxy()
function AddDialysiscard() {
  const newObj = {
    ...
    edit: true,
    add: true,
  } as unknown as PatientCardsView
...
async function saveRow(formEl: FormInstance | undefined, row) {
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      loading.value = true
      const { success } = !row.id ? await patientCardsServiceProxy.patientCardsPOST2(row) : await patientCardsServiceProxy.patientCardsPOST(row.id, row)
      if (success) {
        getCardsInfo()
...
async function switchChange(row) {
  const { success } = await patientCardsServiceProxy.switch(row.id)
  if (success) {
    getCardsInfo()
    showNotify({ type: 'success', message: '保存成功' })
  }
...
async function deleteRow(row) {
  ElMessageBox.confirm('此操作将永久删除该数据, 是否继续?', '提示', {
    ...
    const result = await patientCardsServiceProxy.delete7(row.id)
    if (result.success) {
      showNotify({ type: 'success', message: '删除成功' })
      getCardsInfo()
    }
...
async function getCardsInfo() {
  const { success, data } = await patientCardsServiceProxy.byPatientId(patient.id)

  ...
  <div class="p-2">
    <div>
      ...
      <el-table :data="tableData" border>
        <el-table-column label="住院卡号" prop="cardNumber">
          <template #default="scope">
            <el-form v-if="scope.row.edit || scope.row.add" ref="ruleFormRef" :model="scope.row" :rules="ruleCardNumber" size="small">
              <el-form-item prop="hospitalNumberIn">
                <el-input v-model="scope.row.hospitalNumberIn" maxlength="32" size="small" />
              </el-form-item>
            </el-form>
            <span v-else>{{ scope.row.hospitalNumberIn }}</span>
          </template>
        </el-table-column>
...
import type { PatientHospitalNumberInView } from '@/services/PatientServiceProxies'
import { PatientHospitalNumberInServiceProxy, PatientView } from '@/services/PatientServiceProxies'

const { patient } = defineProps({
  patient: PatientView,
})
const tableData = ref<PatientHospitalNumberInView[]>([])
const ruleFormRef = ref<FormInstance>()
const loading = ref(false)
const rowRecord = ref({})

const ruleCardNumber = reactive<FormRules<PatientHospitalNumberInView>>({
  hospitalNumberIn: [
...
onMounted(() => {
  getPatientHospitalNumberIn()
})

const patientHospitalNumberInServiceProxy = new PatientHospitalNumberInServiceProxy()
function AddDialysiscard() {
  const newObj = {
    ...
    edit: true,
    add: true,
  } as unknown as PatientHospitalNumberInView
...
async function saveRow(formEl: FormInstance | undefined, row) {
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      loading.value = true
      const { success } = !row.id ? await patientHospitalNumberInServiceProxy.patientHospitalNumberInPOST2(row) : await patientHospitalNumberInServiceProxy.patientHospitalNumberInPOST(row.id, row)
      if (success) {
        getPatientHospitalNumberIn()
...
async function switchChange(row) {
  const { success } = await patientHospitalNumberInServiceProxy.switch2(row.id)
  if (success) {
    getPatientHospitalNumberIn()
    showNotify({ type: 'success', message: '保存成功' })
  }
...
async function deleteRow(row) {
  ElMessageBox.confirm('此操作将永久删除该数据, 是否继续?', '提示', {
    ...
    const result = await patientHospitalNumberInServiceProxy.delete25(row.id)
    if (result.success) {
      showNotify({ type: 'success', message: '删除成功' })
      getPatientHospitalNumberIn()
    }
...
async function getPatientHospitalNumberIn() {
  const filter = {
    PageIndex: 1,
    PageSize: 1000,
  }
  const { success, data } = await patientHospitalNumberInServiceProxy.patientGET5(patient.id, JSON.stringify(filter))
